...
name: pull-request
version: 1.0.0
description: "[tddak/TDD 강제] 커밋 히스토리 분석 + 테스트 결과 첨부 PR 생성. verify 게이트 결과를 PR 본문에 포함. 일반 PR은 ttutak:pull-request 사용."
...
> **이 스킬**: pull-request — 테스트 결과 첨부 PR 생성
> **혼동 주의**: ttutak:pull-request와 거의 동일. PR 본문에 verify 게이트 결과(테스트 통과 수)가 추가됨.
> **호출 시 주의**: 이 스킬 내에서 다른 스킬을 호출할 때 반드시 `tddak:` 접두사를 사용한다.

현재 브랜치의 커밋 히스토리를 분석하여 PR을 자동 생성한다.

**TDD 차별점**: PR 본문의 Checklist에 verify 게이트 통과 증거(테스트 통과 수, 실행 시각)를 첨부한다.
...
1. `.claude/config.json`의 `notifications.slack` 확인
   - `enabled`가 `false`이거나 `webhookUrl`이 비어있으면:
     "Slack 알림이 설정되어 있지 않습니다. `/tddak:setup`을 실행하면 웹훅을 연동할 수 있습니다." 안내 후 건너뜀
