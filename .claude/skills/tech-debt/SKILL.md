...
name: tech-debt
version: 1.0.0
description: "[tddak/TDD 강제] 기술 부채 분석 + 우선순위 로드맵 (읽기 전용). 테스트 부채 가중치 상향. 일반 부채 분석은 ttutak:tech-debt 사용."
...
> **이 스킬**: tech-debt — 기술 부채 분석 (테스트 부채 가중치 상향)
> **혼동 주의**: ttutak:tech-debt와 거의 동일. 테스트 부채(test type) 가중치만 상향됨.
> **호출 시 주의**: 이 스킬 내에서 다른 스킬을 호출할 때 반드시 `tddak:` 접두사를 사용한다.

코드베이스의 기술 부채를 유형별로 분석하고, 심각도와 수정 비용을 기반으로 우선순위 로드맵을 제공한다.

**TDD 가중치 차별점**: 우선순위 산정 시 `test` 유형 부채의 가중치를 다른 유형보다 상향 적용한다.
