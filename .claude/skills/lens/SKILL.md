...
name: lens
version: 2.0.0
description: "[tddak/TDD 강제] 코드에서 비즈니스 정책 탐지 + 영향도 분석 (읽기 전용). 테스트 영향도 추가. 일반 lens는 ttutak:lens 사용."
...
> **이 스킬**: lens — 비즈니스 정책 탐지 + 테스트 영향도
> **혼동 주의**: ttutak:lens와 거의 동일. 영향도 분석 시 테스트 영향도가 추가됨.
...
**TDD 차별점**: 영향도 분석 시 정책 변경이 영향을 미치는 **테스트 케이스 수와 위치**를 추가 보고한다.
